...
class MonotonicAggregate(
    ...
    child: SparkPlan)
    extends MonotonicAggregatePartial(
  ...
  override def requiredChildDistribution: List[Distribution] = {
    if (groupingExpressions == Nil) {
      ...
      if (partitioning == Nil) {
        ...
        ClusteredDistribution(
          partitioning.zip(output).filter(_._1 == 1).map(_._2)) :: Nil
      }
    }
  }
  ...
  def isMin(): Boolean =
    (nonCompleteAggregateExpressions ++ completeAggregateExpressions).forall(
      _.aggregateFunction.isInstanceOf[MMin])

  def isMax(): Boolean =
    (nonCompleteAggregateExpressions ++ completeAggregateExpressions).forall(
      _.aggregateFunction.isInstanceOf[MMax])
  ...
  override def outputPartitioning: Partitioning = {
    ...
    if (partitioning == Nil) {
      ...
      HashPartitioning(partitioning.zip(output).filter(_._1 == 1).map(_._2),
                       numPartitions)
    }
  }

  def execute(_allRDD: AggregateSetRDD,
              _cachingFun: (RDD[_] => Unit)): RDD[InternalRow] = {
    allRDD = _allRDD
    cachingFun = _cachingFun
    if (child.isInstanceOf[Exchange] && child.children.head
          .isInstanceOf[MonotonicAggregate]) {
      child.children.head.asInstanceOf[MonotonicAggregate].cachingFun =
        cachingFun
    }
  ...
  protected override def doExecute(): RDD[InternalRow] =
    attachTree(this, "execute") {
      // val aggregateFunction = completeAggregateExpressions.head.
      //                         asInstanceOf[MonotonicAggregateFunction]
      // not partial, then full - merge into exising rdd
      // first iteration - allRDD will be null
      if (allRDD == null) {
        if (!child.outputPartitioning.satisfies(
              this.requiredChildDistribution.head)) {
          throw new SparkException(
            "There is a missing exchange operator which" +
              " should have repartitioned the input rdd!")
        }
  ...
  def getAggregationIterator(
      iter: Iterator[InternalRow],
      aggregateStore: UnsafeFixedWidthMonotonicAggregationMap)
    : TungstenMonotonicAggregationIterator = {
  ...
  def generateResultProjection(): (UnsafeRow, UnsafeRow) => UnsafeRow = {
    ...
    def initializeAllAggregateFunctions(
    ...
    val allAggregateFunctions: Array[AggregateFunction] =
