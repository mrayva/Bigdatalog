...
import edu.ucla.cs.wis.bigdatalog.interpreter.relational.argument.{
  AggregateArgument,
  AliasedArgument,
  AliasedVariable
}
...
/*
 * This class converts a plan from DeAL (Deductive Application Language) logical operators in a
 * top-down manner into a Catalyst logical plan of relational and recursive (BigDatalog) operators.
 *
 * To use another front-end Compiler X, start by replacing the use of DeAL logical operators here
 * with their equivalent operators produced by Compiler X.
 */
class LogicalPlanGenerator(operatorProgram: OperatorProgram,
                           bigDatalogContext: BigDatalogContext) {
  ...
  val gpsi: GeneralizedPivotSetInfo = {
    if (!operatorProgram.getArguments.hasConstant) {
      new GeneralizedPivotSetInfo(
        GeneralizedPivotingSolver.getGeneralizedPivotSet(
          operatorProgram.getProgramRules),
        operatorProgram)
    } else null
  }

  def generateSparkProgram: BigDatalogProgram = {
    if (operatorProgram.getRoot == null) {
      return null
    }

    new BigDatalogProgram(bigDatalogContext,
                          getPlan(operatorProgram.getRoot),
                          operatorProgram)
  }
  ...
  def getPlan(operator: Operator,
    ...
    operator.getOperatorType match {
      ...
      case OperatorType.RECURSIVE_CLIQUE |
        ...
        // we get the schema and register the table in the catalog so recursive relations planned
        // after this node can the same table
        val schema = StructType(
          cliqueOperator.getArguments.asScala.toList
            .map(arg =>
              arg match {
                case v: Variable => v
                case aa: AliasedArgument =>
                  aa.getArgument.asInstanceOf[Variable]
            })
            .map(v =>
              StructField(v.toStringVariableName,
                          Utilities.getSparkDataType(v.getDataType()),
                          false)))

        bigDatalogContext
          .internalCreateDataFrame(
            bigDatalogContext.sparkContext.emptyRDD[InternalRow],
            schema)
          .registerTempTable(cliqueOperator.getName)

        val output = getOutput(cliqueOperator, false)
        var partitioning = if (gpsi == null) {
          Nil
        } else gpsi.getGPSForRecursion(cliqueOperator.getName, cliqueOperator.getArity)
        ...
        val exitRulesPlan: LogicalPlan = operator.getOperatorType match {
          ...
          case OperatorType.MUTUAL_RECURSIVE_CLIQUE =>
            cliqueOperator.getExitRulesOperator match {
              case exitRulesOperator: CliqueOperator =>
                getPlan(exitRulesOperator, recursivePlanDetails)
              case other =>
        ...
        val recursiveRulesPlan = getPlan(
          cliqueOperator.getRecursiveRulesOperator,
          recursivePlanDetails)

        val recursion = cliqueOperator.getEvaluationType match {
          case EvaluationType.MonotonicSemiNaive =>
            AggregateRecursion(cliqueOperator.getName,
          ...
          case EvaluationType.SemiNaive => {
            if (operator.getOperatorType == OperatorType.MUTUAL_RECURSIVE_CLIQUE) {
              MutualRecursion(cliqueOperator.getName,
                              ...
                              recursiveRulesPlan,
                              partitioning)
            } else Recursion(cliqueOperator.getName,
                        cliqueOperator.isLinearRecursion,
                        exitRulesPlan,
                        recursiveRulesPlan,
                        partitioning)
        ...
        if (operator.getArguments.asScala.exists(_.isInstanceOf[AliasedArgument])) {
      ...
      case OperatorType.UNION =>
        val childPlans = operator
          .getChildren()
          .map(child => getPlan(child, recursivePlanDetails))
          .toBuffer

        // if the UNION operator has a constant, we need to push it into the projection below it
        if (operator.getArguments().hasConstant) {
          var count = -1
          val constants = operator
            .getArguments().asScala
            .filter(p => p.isConstant)
            .map(constant => constant.asInstanceOf[DbTypeBase])
            .map(dbType => {
              count += 1
              UnresolvedAlias(
                Alias(Literal.create(
                        Utilities.getDbTypeBaseValue(dbType),
                        Utilities.getSparkDataType(dbType.getDataType)),
                      s"c_$count")())
            })
          ...
          for (i <- 0 until childPlans.size) {
            childPlans(i) match {
              case p: Project if (operator.getArity != p.projectList.size) => {
                childPlans.update(i, Project(p.projectList ++ constants, p.child))
              }
              case _ =>
            }
          }
        ...
        if (bigDatalogContext.getConf.getBoolean(
              "spark.datalog.uniondistinct.enabled",
              true)) {
          Distinct(plan)
        } else plan
      case OperatorType.JOIN =>
        ...
        operator
          .asInstanceOf[JoinOperator]
          .getConditions.asScala
          .foreach(jc => {
            val leftOperator =
              getRelation(operator.getChild(jc.leftRelationIndex))
            val rightOperator =
              getRelation(operator.getChild(jc.rightRelationIndex))
            joinConditions += new JoinCondition(leftOperator.getName,
                                                jc.getLeft.toString,
                                                rightOperator.getName,
                                                jc.getRight.toString)
          })
        ...
        for (i <- 1 until childPlans.size) {
          ...
          if (operator.getChild(i).getOperatorType == OperatorType.NEGATION) {
            val negationOperator =
              operator.getChild(i).asInstanceOf[NegationOperator]
            rightPlan = childPlans(i)
            ...
            negationOperator
              ...
              .map(x => {
                if (key == null) {
                  IsNull( UnresolvedAttribute(
                        x.getRight.asInstanceOf[Variable].toStringVariableName))
                } else IsNull( UnresolvedAttribute(
                      key + "." + x.getRight
                        .asInstanceOf[Variable]
                        .toStringVariableName))
              })
              .foreach(expr => {
            ...
            plan = Filter(
              ...
              Join(plan,
                   rightPlan,
                   LeftOuter,
                   getJoinCondition(operatorKey, used, joinConditions)))
          } else {
            rightPlan = childPlans(i)
            key = getRelationAlias(rightPlan)

            // since we're building top-down, we need to identify operators that
            // are recursion so we can optimize joins
            ...
            var preferredJoinType = bigDatalogContext.getConf
              .get("spark.datalog.jointype", "")
              .toLowerCase
            ...
            if (preferredJoinType.equals("shuffle")) {
              ...
              if (isRecursive(operator.getChild(i - 1)) && !isRecursive(
                    operator.getChild(i))) {
                rightPlan = CacheHint(rightPlan)
              } else if (!isRecursive(operator.getChild(i - 1)) && isRecursive(
                           operator.getChild(i))) {
                plan = CacheHint(plan)
              }
            ...
            plan = Join(plan,
                        rightPlan,
                        Inner,
                        getJoinCondition(key, used, joinConditions))
          }
          used += key
        }
        plan
      case OperatorType.PROJECT =>
        ...
        if (operator
              ...
              .isDistinct && cliqueOperatorStack.isEmpty) {
      ...
      case OperatorType.AGGREGATE | OperatorType.AGGREGATE_FS =>
        ...
        for (arg2 <- operator.getArguments.asScala) {
          if (!usedArguments.contains(arg2)) {
            usedArguments += arg2
            arg2 match {
              case aa: AliasedArgument => {
                aliasName =
                  aa.getAlias.asInstanceOf[Variable].toStringVariableName
                arg = aa.getArgument
              }
              case other => {
                arg = arg2
            ...
            arg match {
              case aa: AggregateArgument => {
                def getAAExpression(t: Argument): Seq[Expression] = {
                  t match {
                    case v: Variable =>
                      Seq(
                        toExpressionFromVariableQualified(
                          operator.getChildren)(v))
                    case d: DbTypeBase =>
                      Seq(
                        Literal.create(
                          Utilities.getDbTypeBaseValue(d),
                          Utilities.getSparkDataType(d.getDataType)))
                    case f: InterpreterFunctor =>
                      f.getArguments.innerArguments.flatMap(getAAExpression(_))
                    case other => null
                  }
                }

                val exprs: Seq[Expression] = getAAExpression(aa.getTerm)
                val aggregateName = if (aa.getName.equals(COUNT_DISTINCT)) {
                  "count"
                } else aa.getName
                aggregate = UnresolvedFunction(
                  aggregateName,
                  exprs,
                  aa.getName.equals(COUNT_DISTINCT))
                aggregateExpressions += UnresolvedAlias(
                  Alias(aggregate, aliasName)())
              }
              case v: Variable =>
                if (!v.isAnonymous) {
                  groupByArguments +=
              ...
              case d: DbTypeBase =>
                groupByArguments += UnresolvedAlias(
                  Alias(
                    Literal.create(Utilities.getDbTypeBaseValue(d),
                                   Utilities.getSparkDataType(d.getDataType)),
                    s"c_$count")())
            }
            count += 1
        ...
        val plan: LogicalPlan = if (groupByArguments.isEmpty) {
          ...
          if (operator.getOperatorType == OperatorType.AGGREGATE) {
            Aggregate(groupByArguments,
                      groupByArguments ++ aggregateExpressions,
                      subPlan)
          } else {
            var partitioning =
              getPartitioning(operator.getName, operator.getArity)
            if (partitioning eq Nil) {
              partitioning = getDefaultPartitioning(operator.getArity)
            }
            MonotonicAggregate(groupByArguments,
                               groupByArguments ++ aggregateExpressions,
                               subPlan,
                               partitioning)
          }
      ...
      case OperatorType.FILTER =>
        ...
        var expressions: Expression = null
        val exprs = operator
          .asInstanceOf[FilterOperator]
          .getExpressions.asScala
          .map(
            c =>
              toExpression(
                c,
                toExpressionFromVariableQualified(operator.getChildren)))
          .foreach(expr => {
            expressions =
              if (expressions == null) { expr } else And(expressions, expr)
          })

        Filter(expressions, subPlan)
      case OperatorType.BASE_RELATION =>
        var relation: LogicalPlan =
          UnresolvedRelation(TableIdentifier(operator.getName), None)
        val operatorName = operator.getName()
        if (recursivePlanDetails.containsBaseRelation(operatorName)) {
          val name = operator.getName + this.subQueryCounter.getAndIncrement()
          relation =
            Subquery(name, Project(Seq(UnresolvedStar(None)), relation))
          operator.setName(name)
          markRenamed(operator, operator.getName())
        }
      ...
      case OperatorType.RECURSIVE_RELATION =>
        val table = bigDatalogContext.catalog.lookupRelation(
          TableIdentifier(operator.getName))
        if (table == null) {
          throw new SparkException(
            "No recursive relation with name '" + operator.getName + "'")
        }

        // we resolve these attributes here since this is this is a virtual table
        val output = operator.getArguments.asScala.map(arg => {
          val variable = arg.asInstanceOf[Variable]
          AttributeReference(variable.getName,
                             Utilities.getSparkDataType(variable.getDataType),
                             false)()
        })
        ...
        val relation: LogicalPlan =
          if (recursiveRelationNames.contains(operator.getName)) {
            val nonLinear = NonLinearRecursiveRelation(operator.getName,
                                                       output,
                                                       partitioning)
            recursivePlanDetails.recursiveRelations
              .put(nonLinear.name, nonLinear)
            // if we use a broadcast join, this relation could be broadcast
            // before the recursion operator has a chance to set it
            ...
            val matchingClique =
              cliqueOperatorStack.filter(_.getName.equals(operator.getName))
            if (matchingClique.nonEmpty &&
                matchingClique.forall(
                  _.getEvaluationType == EvaluationType.MonotonicSemiNaive)) {
              val aggregateRelation =
                AggregateRelation(operator.getName, output, partitioning)
              recursivePlanDetails.aggregateRelations
                .put(aggregateRelation.name, aggregateRelation)
              aggregateRelation
            } else {
              val linear =
                LinearRecursiveRelation(operator.getName, output, partitioning)
      ...
      case OperatorType.SORT =>
        ...
        val sortExprs = sortOperator.getSortOrders.asScala.map(so => {
          val attr = output(so.index)
          if (so.ascending) {
            SortOrder(attr, Ascending)
          } else SortOrder(attr, Descending)
        })
      ...
      case OperatorType.TUPLE =>
        val output = operator.getArguments.asScala.map(x => {
          val arg = x.asInstanceOf[AliasedArgument].getAlias
          AttributeReference(arg.toString,
                             Utilities.getSparkDataType(arg.getDataType),
                             false)()
        })

        val values: Array[Any] = operator.getArguments.asScala
          .map(arg =>
            arg match {
              case aa: AliasedArgument => {
  ...
  def getOutput(operator: Operator,
                usePrefix: Boolean = true): Seq[NamedExpression] = {
    ...
    val output = operator.getArguments.asScala.map(arg => {
      arg match {
        ...
        case d: DbTypeBase => {
          count += 1
          Alias(Literal.create(Utilities.getDbTypeBaseValue(d),
                               Utilities.getSparkDataType(d.getDataType)),
                s"c_$count")()
        }
        case aa: AliasedArgument => {
          ...
          UnresolvedAlias(
            Alias(toExpression(aa.getArgument, variableFunc), v.getName)())
        }
        case av: AliasedVariable =>
          UnresolvedAlias(
            Alias(toExpression(av.getVariable, variableFunc), av.getAlias)())
      }
    })
    output
  }
  ...
  def getRelation(operator: Operator): Operator = {
  ...
  def toExpressionFromVariable(operators: Seq[Operator])(
      v: Variable): UnresolvedAttribute =
    UnresolvedAttribute(v.getName)

  def toExpressionFromVariableQualified(operators: Seq[Operator])(
      v: Variable): UnresolvedAttribute = {
    getSourceOperator(v, operators) match {
      case Some(sourceOperator) =>
        getPrefix(sourceOperator) match {
          case Some(prefix) =>
            UnresolvedAttribute(prefix + "." + v.toStringVariableName)
  ...
  def toExpression(
      arg: Argument,
      varNameFunc: (Variable => UnresolvedAttribute)): Expression = {
    arg match {
  ...
  def getJoinCondition(
    ...
    for (jc <- joinConditions) {
      ...
      if (used.contains(leftName) && rightName.equals(key)) {
        matchingJoinConditions += EqualTo(
          UnresolvedAttribute(Seq(leftName, jc.leftRelationColumnName)),
  ...
  private def isRecursive(operator: Operator): Boolean = {
    ...
    // if any child node of this subtree is a producer inside a recursion,
    // this operator is part of a recursion
    return operator.getChildren.exists(isRecursive(_))
  }

  private def getPartitioning(name: String, arity: Int): Seq[Int] = {
    val userDefined =
      bigDatalogContext.getConf.get("spark.datalog.partitioning." + name, "")
    ...
    if (userDefined.length == 0) {
      ...
      userDefined
        .substring(1, userDefined.length - 1)
        .split(",")
        .map(_.trim.toInt)
    }
  }

  private def getSourceOperator(variable: Variable,
                                operators: Seq[Operator]): Option[Operator] = {
  ...
  private def getSourceOperator(variable: Variable,
    ...
    operator.getOperatorType match {
      case OperatorType.PROJECT => {
        operator.getArguments.asScala
          .filter(_.isInstanceOf[AliasedArgument])
          .foreach(arg => {
      ...
      case OperatorType.RECURSIVE_CLIQUE |
          OperatorType.MUTUAL_RECURSIVE_CLIQUE | OperatorType.UNION |
          OperatorType.AGGREGATE | OperatorType.AGGREGATE_FS |
          OperatorType.BASE_RELATION | OperatorType.RECURSIVE_RELATION |
          OperatorType.TUPLE => {
        // if any of these cases are encountered,
        // we have found a producer of the values assigned to the variable - i.e., go no deeper
        val variables = operator.getArguments.asScala
          .filter(_.isInstanceOf[Variable])
          .map(_.asInstanceOf[Variable]) ++
          operator.getArguments.asScala
            .filter(_.isInstanceOf[AliasedArgument])
            .map(
              _.asInstanceOf[AliasedArgument].getAlias.asInstanceOf[Variable])
