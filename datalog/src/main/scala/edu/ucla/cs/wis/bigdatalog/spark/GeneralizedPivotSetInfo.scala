...
class GeneralizedPivotSetInfo(val gps: GeneralizedPivotSet,
                              val program: OperatorProgram)
    extends Serializable {

  def isEmpty : Boolean = gps.isEmpty
  ...
  def getGPSForBaseRelation(recursivePredicateName: String,
    ...
    val rules =
      programRules.getRules(recursivePredicateName, recursivePredicateArity)

    // get recursive predicate list so we can find the exit rules
    val recursivePredicateNames = HashSet.empty[String]
    for (clique <- programRules.getCliques.asScala)
      for (dp <- clique.getDerivedPredicates.asScala)
    ...
    for (rule <- rules.asScala) {
      for (literal <- rule.getBody.asScala)
        if (recursivePredicateNames.contains(
              literal.getPredicateName + "|" + literal.getArity)) {
          found = true
        }
    ...
    // for each exit rule, find the mapping from the head argument to the body literal argument,
    // if present
    // TODO - arithmetic expressions will not work - this version only expects to find the argument
    // in a base predicate
    if (exitRules.size == 1) {
      val rule = exitRules.head
      // if exit rule is result of rewritting - such as w/ aggregates, use operator tree to
      // identify arguments in base relation to filter by
      if (rule.isResultOfRewrite || rule.isRewritten) {
        ...
        for (brl <- baseRelationLiterals) {
          for (arg <- brl.getArguments().asScala) {
            var found = false
  ...
  def getGPSForBaseRelationFromPlan(recursivePredicateName: String,
    ...
    val recursivePredicateGPS = gps.get(recursivePredicateName)
    val recursion = getCliqueOperator(program.getRoot,
                                      recursivePredicateName,
                                      recursivePredicateArity)
    val baseRelationPredicate = getBaseRelation(recursion.getExitRulesOperator,
                                                basePredicateName,
                                                basePredicateArity)

    val recursivePredicateNonZeroCoefficientArgs =
      recursivePredicateGPS.zipWithIndex
        .filter(x => x._1 > 0)
        .map(x => (x._1, recursion.getArgument(x._2))) // (coefficient, argument)
        .map(x => {
          (x._1, {
            x._2 match { // (coefficient, argument)
    ...
    val args = baseRelationPredicate.getArguments.asScala.zipWithIndex
      .foreach(p => {
        if (recursivePredicateNonZeroCoefficientArgs.contains(p._1)) {
          basePredicateGPS(p._2) =
            recursivePredicateNonZeroCoefficientArgs.get(p._1).get
        }
      })
  ...
  def getBaseRelation(operator: Operator,
                      basePredicateName: String,
                      basePredicateArity: Int): Operator = {
    if (operator.getOperatorType == OperatorType.BASE_RELATION) {
      if (operator.getName.equals(basePredicateName) && (operator.getArity == basePredicateArity)) {
        operator
      } else null
    } else {
      val childResults = operator.getChildren
        .map(child =>
          getBaseRelation(child, basePredicateName, basePredicateArity))
        .filter(x => x != null)
      if (childResults.length > 0) childResults.head else null
    }
  }

  def getCliqueOperator(operator: Operator,
                        recursivePredicateName: String,
                        recursivePredicateArity: Int): CliqueOperator = {
    operator match {
      case co: CliqueOperator => {
        if (co.getName.equals(recursivePredicateName) && co.getArity == recursivePredicateArity) {
          co
        } else {
          var result = getCliqueOperator(co.getExitRulesOperator,
                                         recursivePredicateName,
                                         recursivePredicateArity)
          if (result == null) {
            result = getCliqueOperator(co.getRecursiveRulesOperator,
                                       recursivePredicateName,
                                       recursivePredicateArity)
          }
          result
      ...
      case other => {
        val childResults = other.getChildren
          .map(
            child =>
              getCliqueOperator(child,
                                recursivePredicateName,
                                recursivePredicateArity))
          .filter(x => x != null)
        if (childResults.length > 0) childResults.head else null
